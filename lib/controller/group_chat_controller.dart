...
class GroupChatController {
  ...
  static const String URL =
      "https://script.google.com/macros/s/AKfycby_AazDJI9CjFfx9R7tY2xw06SRoqn_9hCAEMXu9SE85o-7udw/exec";
  ...
  void submitForm(
      GroupChatModal feedForm, void Function(String) callback) async {
    try {
  ...
  Future<List<GroupChatModal>> getFeedList() async {
    return await http.get(URL).then((response) {
      var jsonFeedResult = convert.jsonDecode(response.body) as List;
      print(
          "response: ${jsonFeedResult.map((v) => GroupChatModal.fromJson(v).toJson())}");

      return jsonFeedResult
          .map((json) => GroupChatModal.fromJson(json))
          .toList();
    });
  }
}
